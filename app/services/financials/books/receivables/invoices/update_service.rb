class Financials::Books::Receivables::Invoices::UpdateService
  ...
  def update_invoice
    ...
    @invoice.total_addition_amount = @items.select{|i| i.kind == "addition"}.map{|i| i.amount}.sum
    ...
    @invoice.total_invoice_amount = @invoice.total_income_amount - @invoice.total_discount_amount + @invoice.total_addition_amount
    @invoice.total_amount = @invoice.total_income_amount + @invoice.total_refund_amount + @invoice.total_addition_amount - @invoice.total_discount_amount - @invoice.total_reversal_amount - @invoice.total_error_amount
