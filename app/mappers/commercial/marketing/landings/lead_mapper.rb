class Commercial::Marketing::Landings::LeadMapper < BaseMapper
  ...
  def self.map_lead_gen(model)
    ...
    obj = obj.merge({ "sql" => Commercial::Sale::Lead::Source.where(source_id: model.id, source_type: "landing_conversion").first.present? })
    ...
    obj = obj.merge({ "source" => "landing" })
    obj = obj.merge({ "source_id" => model.id })
    obj = obj.merge({ "source_type" => "landing_conversion" })
    obj = obj.merge({ "source_pretty" => "Landing" })
