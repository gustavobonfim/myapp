class Commercial::Sales::Opportunities::Entities::Create
  ...
  def initialize(params)
    @opportunity_params = params.require(:opportunity).permit(:stage, :status, :source, :started_at, :notes)
    @lead_params = params.require(:lead).permit(:id)
  ...
  def save
    ...
    ActiveRecord::Base.transaction do
      if @valid 
        ...
        ::Commercial::Sales::Leads::UpdateLeadStatusService.new(@lead_params[:id], "in_process").update_lead
        ::Commercial::Sales::Opportunities::CreateOpportunityJourneyService.new(@opportunity).save_journey
        ::Commercial::Sales::Opportunities::CreateOpportunityLeadService.new(@opportunity.date_id, @opportunity.id, @lead_params[:id]).save_lead
