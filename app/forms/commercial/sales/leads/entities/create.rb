class Commercial::Sales::Leads::Entities::Create
  ...
  def initialize(params)
    @source_params = params.require(:source).permit(:source, :source_id, :source_type, :name, :email, :prefix, :phone)
    @lead_params = params.require(:lead).permit(:name, :primary_source, :council_type, :council_number, :council_state, :link, :status, :notes,
                                                :email, :prefix, :phone)
    ...
    if @source_params[:source] == "event" || @source_params[:source] == "landing" || @source_params[:source] == "session"
  ...
  def save
    ...
    ActiveRecord::Base.transaction do
      if @valid 
        ...
        @source_params = @source_params.merge({ "name" => @lead.name })
        @source_params = @source_params.merge({ "email" => @lead.email })
        @source_params = @source_params.merge({ "prefix" => @lead.prefix })
        @source_params = @source_params.merge({ "phone" => @lead.phone })
