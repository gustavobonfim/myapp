...
export default class extends Controller {
  ...
  connect() {
    this.leads = []
    this.controllerName = `commercial--dashboards--sources--events`
    this.itemsOnPage = 20
    this.pageNumber = 0
    this.numberPages = []
  ...
  doIndexListHtml() {
    var html = `<div class="card" style="width:100%;display:relative;" data-controller="app--helpers--search" data-target="${this.controllerName}.mainCard" data-action="resize@window->${this.controllerName}#layout">
                  <div class="card-header d-flex align-items-center card-header-table-list f-065">
                    <h6 class="card-title display-4 card-title-table-list">Leads de Eventos</h6>
  ...
  getLeads() {
    const data = { attendee: { active: true }, current_user: { current_user_id: this.application.current_user.id } }
    const url = "/commercial/marketing/leads/generations/list_attendee"
